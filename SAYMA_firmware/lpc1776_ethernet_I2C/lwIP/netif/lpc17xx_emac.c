...
#define DESCRIPTORS_LOCATION_SECTION RamPeriph32
/* MII Mgmt Configuration register - Clock divider setting */
const uint8_t EMAC_clkdiv[] =  { 4, 6, 8, 10, 14, 20, 28, 36, 40, 44, 48, 52, 56, 60, 64 };

static volatile RX_Desc Rx_Desc[EMAC_NUM_RX_FRAG] __BSS(DESCRIPTORS_LOCATION_SECTION);
...
static volatile __attribute__ ((aligned (8))) RX_Stat Rx_Stat[EMAC_NUM_RX_FRAG] __BSS(DESCRIPTORS_LOCATION_SECTION);
#endif

/** Tx Descriptor data array */
static volatile TX_Desc Tx_Desc[EMAC_NUM_TX_FRAG] __BSS(DESCRIPTORS_LOCATION_SECTION);
/** Tx Status data array */
static volatile TX_Stat Tx_Stat[EMAC_NUM_TX_FRAG] __BSS(DESCRIPTORS_LOCATION_SECTION);
...
/** Rx buffer data */
static volatile uint32_t rx_buf[EMAC_NUM_RX_FRAG][EMAC_ETH_MAX_FLEN>>2] __BSS(DESCRIPTORS_LOCATION_SECTION) = { { 0 }, { 0 } };
/** Tx buffer data */
static volatile uint32_t tx_buf[EMAC_NUM_TX_FRAG][EMAC_ETH_MAX_FLEN>>2] __BSS(DESCRIPTORS_LOCATION_SECTION) = { { 0 }, { 0 } };
